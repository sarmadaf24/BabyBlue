...
echo "ServerName ${BB_DOMAIN}" > /etc/apache2/conf-available/servername.conf; a2enconf servername >/dev/null 2>&1 || true
...
for f in /etc/apache2/sites-enabled/*-le-ssl.conf /etc/apache2/sites-enabled/000-default-le-ssl.conf; do [ -e "$f" ] && a2dissite "$(basename "$f")" >/dev/null 2>&1 || true; done
APP="/opt/openwisp2"
...
python3 -m venv "$APP/venv"; . "$APP/venv/bin/activate"; pip install --upgrade pip >/dev/null
if [ -f requirements.txt ]; then pip install -r requirements.txt; else pip install openwisp-controller openwisp-ipam openwisp-monitoring openwisp-users netjsonconfig django psycopg2-binary gunicorn; fi
...
if [ -f inventory/openwisp_env.sh ]; then install -m 600 -D inventory/openwisp_env.sh /root/babyblue_inventory/openwisp_env.sh; sed -i "s#^OPENWISP_URL=.*#OPENWISP_URL=https://${BB_DOMAIN}#g" /root/babyblue_inventory/openwisp_env.sh || true; fi
if [ -f state/db/openwisp.dump ]; then systemctl enable postgresql >/dev/null 2>&1 || true; systemctl start postgresql >/dev/null 2>&1 || true; sudo -u postgres psql -tc "SELECT 1 FROM pg_database WHERE datname=openwisp;" | grep -q 1 || sudo -u postgres createdb openwisp; sudo -u postgres pg_restore -j 2 -c -d openwisp state/db/openwisp.dump || true; fi
cd "$APP"; "$APP/venv/bin/python" manage.py migrate --noinput || true
...
echo "HTTP_STATUS=${HTTP_CODE}"; echo "HTTPS_STATUS=${HTTPS_CODE}"; echo "ADMIN_STATUS=${ADMIN_CODE}"; echo "GUNICORN=${GUNI_STATUS}"
